...
답 : 점진적 타입 시스템​(Gradual Type System)
...
답 : 타입이 정의되어 있지 않은 변수의 값이나 함수의 반환값을 기반으로 타입스크립트가 타입을 자동으로 추론한다.

런타임 이전에 오류를 검사해서 타입에 이상이 없는 지 검사한다. 이 시스템은 마치 정적 타입 시스템과 동적 타입 시스템을 혼합한 것처럼 동작한다.

다만, 때에 따라 함수의 매개변수 타입이나 변수의 타입을 타입스크립트가 제대로 타입추론을 못하는 경우도 있어서 타입 선언을 꼭 해줘야 하는 경우도 있다.
...
​strictNullChecks를 false로 설정한다. 단 안전한 코드 작성을 위해서 true로 설정하는 것이 좋다.
